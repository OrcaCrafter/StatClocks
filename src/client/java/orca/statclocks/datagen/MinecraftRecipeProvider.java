	...
	protected @NotNull RecipeProvider createRecipeProvider (HolderLookup.Provider registryLookup, RecipeOutput exporter) {
		return new RecipeProvider(registryLookup, exporter) {
			...
			public void buildRecipes () {
				...
				Ingredient ingredient = this.tag(ItemTags.HARNESSES);
				
				for (DyeColor dyeColor : DyeColor.values()) {
					Item harness = getHarnessByColor(dyeColor);
					
					String name = "dye_" + BuiltInRegistries.ITEM.getKey(harness).getPath();
					
					DyeItem dyeItem = DyeItem.byColor(dyeColor);
					TransmuteRecipeBuilder.transmute(RecipeCategory.TOOLS, ingredient, Ingredient.of(dyeItem), harness)
						.group("harness_dye")
